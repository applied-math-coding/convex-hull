use crate::models::{NewConvexHull, ConvexHull, NewPoint, Point};
use crate::db;
use crate::schema::{convex_hulls, points};
use diesel::prelude::*;
use diesel::result::Error;

pub fn create_convex_hull(new_convex_hull: NewConvexHull, mut new_point: NewPoint) -> (ConvexHull, Point){  
  let connection = db::create_connection();
   connection.transaction::<(ConvexHull, Point), Error, _>(|| {    
    ...
    new_point.convex_hull_id = convex_hull.id;
...
pub fn get_points(convex_hull_id: i32) -> Point{
  ...
  let convex_hull = convex_hulls::table
    ...
    .unwrap();
  Point::belonging_to(&convex_hull).first(&connection).unwrap()
} 
...
mod tests {
  ...
  fn test_convex_hull_crud(){    
    let (convex_hull, ..) = create_convex_hull(
      ...
      NewPoint { input: json!({"hello": "world"}), output: json!({"hello": "world"}), convex_hull_id: -1}
    );
    get_points(convex_hull.id);
    assert_eq!(get_convex_hulls().iter().filter(|e| e.id == convex_hull.id).collect::<Vec<&ConvexHull>>().len(), 1);
    delete_convex_hull(convex_hull.id);
    assert_eq!(get_convex_hulls().iter().filter(|e| e.id == convex_hull.id).collect::<Vec<&ConvexHull>>().len(), 0);
  }
}